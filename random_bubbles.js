(function bubble() {
    ...
    function moveIt()
    {
        ...
        {            
            ...
            nowY += dy;
                // Calc wrap around
            if (nowX < 0) nowX = 1250 + nowX;
            else          nowX = nowX % 1250;            
            if (nowY < 0) nowY = 575 + nowY;
            else          nowY = nowY % 575;
            ...
            else circs[i].vel = circs[i].vel - .0005;              
    ...
    window.onload = function bubble () {
        ...
        paper = Raphael("canvas", 1250, 575);
        circs = paper.set();
        for (i = 0; i < 30; ++i)
        {
            opa = ran(3,10)/10;
            circs.push(paper.circle(ran(0,1250), ran(0,575), ran(10,30)).attr({"fill-opacity": opa,
                                                                           "stroke-opacity": opa}));
        }
        circs.attr({fill: "#ffffff", stroke: "#39E2FC"});
        moveIt();
